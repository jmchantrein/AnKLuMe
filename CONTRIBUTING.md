...
4. **Validate**: Run `anklume dev lint` — this chains all validators:
   - `ansible-lint` (production profile)
   - `yamllint`
   - `shellcheck` (all `.sh` files)
   - `ruff` (all `.py` files)
5. **Test**: Run `anklume dev test`.
...
- Files in `inventory/`, `group_vars/`, `host_vars/` (generated by `anklume sync`)
- Private IP addresses in non-exempt files

The hook is installed automatically by `anklume setup init` (or `anklume setup hooks`).
