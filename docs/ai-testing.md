...
| `ANKLUME_AI_OLLAMA_URL` | `http://gpu-server:11434` | Ollama API URL |
...
# Set mode
export ANKLUME_AI_MODE=local
export ANKLUME_AI_OLLAMA_URL=http://gpu-server:11434
...
If running inside the admin container, ensure the ai-tools domain is
accessible (requires admin→ai-tools network access).
