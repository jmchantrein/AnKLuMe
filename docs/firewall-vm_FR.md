...
dans `infra.yml`. Le generateur PSOT cree automatiquement la machine `anklume-firewall`
...
La `anklume-firewall` creee automatiquement a : type `vm`, IP `.253` dans le
...
Pour surcharger les valeurs par defaut, ajoutez `anklume-firewall` au domaine anklume :
...
3. Attache le profil a la VM anklume-firewall
...
1. Verifier le forwarding IP : `incus exec anklume-firewall -- sysctl net.ipv4.ip_forward`
2. Verifier les regles nftables : `incus exec anklume-firewall -- nft list ruleset`
