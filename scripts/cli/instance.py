...
def list_(
    ...
    from scripts.cli._instance_list import render_instance_table
    render_instance_table(domain, sort)
...
def _find_project(infra: dict, instance_name: str) -> str | None:
    ...
    for dname, dconf in (infra.get("domains") or {}).items():
        if instance_name in (dconf.get("machines") or {}):
            gv = PROJECT_ROOT / "group_vars" / f"{dname}.yml"
            if gv.is_file():
                with open(gv) as f:
                    return (yaml.safe_load(f) or {}).get("incus_project", dname)
            return dname
    return None
